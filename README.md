...
- **Time:** *Now* means the newest state (freshest captures on the archive side); *All time* spans the archive’s full history — single pages are read across up to a dozen snapshots sampled over their whole capture history; *Custom* opens two calendars to bracket an era.

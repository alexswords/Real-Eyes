...
- **Time:** *Now* means the newest state (freshest captures on the archive side); *All time* spans the archive’s full history — single pages combine a dozen sampled snapshots (catching media hosted anywhere) with a full index sweep of the page’s folder (catching every file ever stored beside it); *Custom* opens two calendars to bracket an era.
- **Deep captures:** with Wayback (or Both) + a time other than Now, a *Deep* toggle appears under Start Scrape. On *This page* it reads every archived capture of the page instead of ~12 samples, catching short-lived media. On *Site folder* / *Entire site* it additionally reads the site's archived HTML pages, surfacing media the archive index alone can't see — files hosted on other domains, or referenced only from markup/CSS. Either way: many more fetches, much slower.
- **Animated GIFs:** when the GIF type toggle is on, an *animated only* sub-toggle appears; flipping it probes the visible GIFs (parsing each file's frame structure) and hides only the confirmed-static ones — GIFs that can't be probed (rate-limited, truncated) stay visible.
- **Spotting outliers across time:** a multi-snapshot page scrape counts how many snapshots each image appears in. Sort by *Rarity* to float one-off images to the top; the list view shows an N/total column and grid cards get a red *rare* badge for images seen in a single snapshot — a fast way to find media that came and went without opening each capture.

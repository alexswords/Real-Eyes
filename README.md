...
- **Deep captures:** with Wayback (or Both) + This page + a time other than Now, a *Deep* toggle appears under Start Scrape — it reads every archived capture of the page instead of ~12 samples, catching short-lived media at the cost of many more fetches.
- **Animated GIFs:** when the GIF type toggle is on, an *animated only* sub-toggle appears; flipping it probes the visible GIFs and hides the static ones.
